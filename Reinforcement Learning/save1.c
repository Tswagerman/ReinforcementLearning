...
int returnMaxProb(double *distribution, int x){
	double *maxValues = (double*) malloc (10000 * (sizeof(int)));
		...
		case 1:
			...
			for(int t = 0; t < runs; t ++){
				for(int a = 0; a < actionSelections; a++){
					...
					int optionG = returnMaxProb(gaussianDistribution, gaussianArmOption);
					if (optionG == 1){
						optimalArmGaussianCnt++;
					}
					double gaussianReward = gaussianDistribution[gaussianArmOption];
					gaussianRewardArray[a] = gaussianReward; //Adding probability reward to reward array
					//Bernoulli
					int bernoulliArmOption = epsilonGreedy(bernoulliDistribution, epsilon);
					//If bernoulliArmOption != bestOption {counter++}
					int optionB = returnMaxProb(bernoulliDistribution, bernoulliArmOption);
					if (optionB == 1){
